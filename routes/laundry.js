...
// customer : 신청한 세탁 서비스 취소하기 (Laundry Data가 삭제가 안됨)
...
router.patch('/laundry/collect/:laundryId', jwtSupplier, async (req, res) => {
  try {
    ...
    const { laundryId } = req.params;
    const laundryStatus = await Laundry.findOne({
      where: { laundryId, status: 0 },
    });
    if (!laundryStatus) {
      return res.status(400).json({ Message: '이미 수거된 세탁물 입니다.' });
    }
    return (
      await Laundry.update({ supplierId, status: 1 }, { where: { laundryId } }),
...
router.get('/laundry/collect/:supplierId', jwtSupplier, async (req, res) => {
  try {
    ...
    if (!currentLaundry) {
      return res.status(400).json({ Message: '수거 접수한 세탁물이 없습니다.' });
    }
